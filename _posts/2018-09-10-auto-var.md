...
Notice there is *no clustering* taking place - the coding space is homogenous. This enables us to perform smooth digit interpolations between styles (keeping the digit type constant), between digits (keeping the style constant), or both. We leave it as a fun exercise for the reader to write the code that accomplishes this.
